"use client"

import { useState, useEffect } from "react"
import { mockUserProfile } from "../data/mockData"
import type { ViewType, BookingForm } from "../types/index"

// Pages
import LandingPage from "../pages/LandingPage"
import SignInPage from "../pages/SignInPage"
import HomePage from "../pages/HomePage"
import SearchPage from "../pages/SearchPage"
import ReservationsPage from "../pages/ReservationsPage"
import ProfilePage from "../pages/ProfilePage"
import PaymentMethodsPage from "../pages/PaymentMethodsPage"
import NotificationsPage from "../pages/NotificationsPage"

// Dialogs
import ProfileDialog from "./dialogs/ProfileDialog"
import SettingsDialog from "./dialogs/SettingsDialog"
import ReservationModal from "./dialogs/ReservationModal"

// Mock Data
const mockParkingSpots = [
  {
    id: 1,
    name: "Downtown Shopping Mall",
    address: "123 Main Street, Downtown",
    status: "available",
    price: 8,
    distance: "0.2 mi",
    rating: 4.5,
    features: ["Covered", "Security", "EV Charging"],
    totalSpots: 150,
    occupiedSpots: 89,
    image: "🏢",
    category: "mall",
  },
  {
    id: 2,
    name: "Business District Plaza",
    address: "456 Corporate Ave",
    status: "available",
    price: 12,
    distance: "0.5 mi",
    rating: 4.2,
    features: ["24/7 Access", "Security", "Valet"],
    totalSpots: 200,
    occupiedSpots: 156,
    image: "🏢",
    category: "office",
  },
  {
    id: 3,
    name: "International Airport",
    address: "789 Airport Blvd",
    status: "available",
    price: 15,
    distance: "2.1 mi",
    rating: 4.3,
    features: ["Shuttle Service", "Long-term", "Security"],
    totalSpots: 500,
    occupiedSpots: 423,
    image: "✈️",
    category: "airport",
  },
  {
    id: 4,
    name: "Concert Arena",
    address: "321 Entertainment District",
    status: "available",
    price: 20,
    distance: "1.2 mi",
    rating: 4.8,
    features: ["Event Parking", "Security", "Pre-book"],
    totalSpots: 300,
    occupiedSpots: 45,
    image: "🎵",
    category: "event",
  },
  {
    id: 5,
    name: "City Hospital",
    address: "555 Medical Center Dr",
    status: "available",
    price: 10,
    distance: "0.8 mi",
    rating: 4.6,
    features: ["Valet Service", "24/7 Access", "Validation"],
    totalSpots: 250,
    occupiedSpots: 198,
    image: "🏥",
    category: "hospital",
  },
  {
    id: 6,
    name: "State University",
    address: "100 Campus Drive",
    status: "available",
    price: 5,
    distance: "1.5 mi",
    rating: 3.9,
    features: ["Student Rates", "Permit Parking", "Bike Racks"],
    totalSpots: 400,
    occupiedSpots: 267,
    image: "🎓",
    category: "university",
  },
]

const mockBookings = [
  {
    id: 1001,
    spotId: 1,
    location: "Downtown Shopping Mall",
    date: "2024-01-15",
    time: "10:00 AM",
    duration: "3 hours",
    status: "active",
    totalCost: 26,
    paymentStatus: "paid",
    paymentMethod: "Visa ****4242",
    userId: "user_123",
    userName: "John Doe",
  },
  {
    id: 1002,
    spotId: 3,
    location: "International Airport",
    date: "2024-01-16",
    time: "6:00 AM",
    duration: "5 days",
    status: "upcoming",
    totalCost: 375,
    paymentStatus: "paid",
    paymentMethod: "Mastercard ****5555",
    userId: "user_123",
    userName: "John Doe",
  },
]

const mockPaymentMethods = [
  { id: "pm_1", type: "card", last4: "4242", brand: "Visa", isDefault: true },
  { id: "pm_2", type: "card", last4: "5555", brand: "Mastercard", isDefault: false },
  { id: "pm_3", type: "paypal", isDefault: false },
]

export default function ParkingApp() {
  const [currentView, setCurrentView] = useState<ViewType>("landing")
  const [searchQuery, setSearchQuery] = useState("")
  const [selectedSpot, setSelectedSpot] = useState<any | null>(null)
  const [selectedBooking, setSelectedBooking] = useState<any | null>(null)
  const [isDarkMode, setIsDarkMode] = useState(false)
  const [isLoggedIn, setIsLoggedIn] = useState(false)
  const [userProfile, setUserProfile] = useState<any>(mockUserProfile)

  // Dialog states
  const [showProfileDialog, setShowProfileDialog] = useState(false)
  const [showSettingsDialog, setShowSettingsDialog] = useState(false)

  // Booking form state
  const [bookingForm, setBookingForm] = useState<BookingForm>({
    date: "",
    time: "",
    duration: 2,
    paymentMethod: "",
  })

  // Dark mode effect
  useEffect(() => {
    document.documentElement.classList.toggle("dark", isDarkMode)
  }, [isDarkMode])

  const handleSignIn = () => {
    setIsLoggedIn(true)
    setCurrentView("home")
  }

  const handleSignOut = () => {
    setIsLoggedIn(false)
    setCurrentView("landing")
  }

  const handleSearch = () => {
    if (searchQuery.trim()) {
      setCurrentView("search")
    }
  }

  const handleSpotSelect = (spot: any) => {
    setSelectedSpot(spot)
  }

  const handleViewReceipt = (booking: any) => {
    setSelectedBooking(booking)
    // Show receipt dialog logic here
  }

  const handleLogoClick = () => {
    setCurrentView("landing")
  }

  const toggleDarkMode = () => {
    setIsDarkMode(!isDarkMode)
  }

  // Render current view
  const renderCurrentView = () => {
    if (!isLoggedIn && currentView !== "landing" && currentView !== "signin") {
      return <SignInPage onViewChange={setCurrentView} onSignIn={handleSignIn} />
    }

    switch (currentView) {
      case "landing":
        return <LandingPage onViewChange={setCurrentView} />

      case "signin":
        return <SignInPage onViewChange={setCurrentView} onSignIn={handleSignIn} />

      case "home":
        return (
          <HomePage
            currentView={currentView}
            searchQuery={searchQuery}
            onSearchChange={setSearchQuery}
            onSearchSubmit={handleSearch}
            onViewChange={setCurrentView}
            onLogoClick={handleLogoClick}
          />
        )

      case "search":
        return (
          <SearchPage
            currentView={currentView}
            searchQuery={searchQuery}
            onSearchChange={setSearchQuery}
            onSearchSubmit={handleSearch}
            onViewChange={setCurrentView}
            onSpotSelect={handleSpotSelect}
          />
        )

      case "reservations":
        return (
          <ReservationsPage currentView={currentView} onViewChange={setCurrentView} onViewReceipt={handleViewReceipt} />
        )

      case "profile":
        return (
          <ProfilePage
            currentView={currentView}
            userProfile={userProfile}
            isDarkMode={isDarkMode}
            onViewChange={setCurrentView}
            onToggleDarkMode={toggleDarkMode}
            onShowProfileDialog={() => setShowProfileDialog(true)}
            onShowSettingsDialog={() => setShowSettingsDialog(true)}
            onSignOut={handleSignOut}
          />
        )

      case "payment-methods":
        return <PaymentMethodsPage currentView={currentView} onViewChange={setCurrentView} />

      case "notifications":
        return <NotificationsPage currentView={currentView} onViewChange={setCurrentView} />

      default:
        return <LandingPage onViewChange={setCurrentView} />
    }
  }

  return (
    <>
      {renderCurrentView()}

      {/* Dialogs */}
      <ProfileDialog
        isOpen={showProfileDialog}
        onClose={() => setShowProfileDialog(false)}
        userProfile={userProfile}
        onSave={setUserProfile}
      />

      <SettingsDialog
        isOpen={showSettingsDialog}
        onClose={() => setShowSettingsDialog(false)}
        isDarkMode={isDarkMode}
        onToggleDarkMode={toggleDarkMode}
        userProfile={userProfile}
        onUpdateProfile={setUserProfile}
      />

      <ReservationModal
        spot={selectedSpot}
        bookingForm={bookingForm}
        onBookingFormChange={setBookingForm}
        onClose={() => setSelectedSpot(null)}
        onContinue={() => {
          // Handle payment flow
          setSelectedSpot(null)
          setCurrentView("reservations")
        }}
      />
    </>
  )
}
